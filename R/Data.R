...
Data.generate <- function(){
  ...
  sample.id1 <- sample(sum(id1), 1000)
  sample.id2 <- sample(sum(id2), 1000)
  sample.id3 <- sample(sum(id3), 1000)
  X1 <- X1[sample.id1,]; X2 <- X2[sample.id2,]; X3 <- X3[sample.id3,]
  dat <- data.frame(rbind(X1, X2, X3), 
                    label = rep(c(1,2,3), each = 1000))
  dat <- dat[sample(3000),]
  return(list(X = as.matrix(dat[,1:4]), label = as.factor(dat$label)))
}
